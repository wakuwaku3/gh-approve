...
export const usePullRequests = (): UsePullRequestsResult => {
  ...
  useEffect(() => {
    if (!token || !targetRepositoryOwner || !targetRepositoryName) {
      setError(
        new Error(
          "GitHub configuration (token, owner, repo) is missing in context."
        )
      );
      setPullRequests([]);
      setLoading(false);
    ...
    const fetchAllPullRequests = async () => {
      ...
      let nextUrl:
        | string
        | null = `https://api.github.com/repos/${targetRepositoryOwner}/${targetRepositoryName}/pulls?state=open&per_page=100&direction=desc`;
      ...
      try {
        for (const prNumber of numbers) {
          ...
          if (!response.ok) {
            ...
            throw new Error(
              `Failed to approve PR #${prIDStr}: ${
                String(errBody.message) || response.statusText
              }`
            );
          }
        }
